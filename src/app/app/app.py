<truncated__content/>"].isin(["N", "L", "H"]), "impact"] = ""
...
# ============================================================================ #
# 2) Status Notifications Tab (simple, no code-enrichment)
# ============================================================================ #
with tabs[1]:
    ...
    if not status_table:
        ...
        if status_df.empty:
            ...
            if "selected_evse_ids" in locals() and selected_evse_ids:
                _set = {str(x) for x in selected_evse_ids}
            ...
            # sort newest -> oldest using AKDT_dt
            ...
            # rename for display (keep AKDT_dt for export)
            display_df = status_df[display_cols].copy()
            if "AKDT_dt" in display_df.columns:
                display_df["Date/Time (AKDT)"] = display_df["AKDT_dt"].dt.tz_convert(AK).dt.strftime("%Y-%m-%d %H:%M:%S")
                display_df = display_df.drop(columns=["AKDT_dt"], errors="ignore")
            ...
            try:
                if AGGRID_AVAILABLE:
                    ...
                    AgGrid(display_df, gridOptions=gob.build(), fit_columns_on_grid_load=True, height=320)
                else:
